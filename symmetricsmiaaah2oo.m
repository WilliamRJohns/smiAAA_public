function [bestbcr,bestw,bcr,z,wj,fz,err] = symmetricsmiaaah2oo(f,Z,tol,normalize,iter,ref)
...
for l=1:iter %Optimization Iterations
disp("iter= "+l);
%Fix num weights
a=A*wh_num;
wden=B*H\a;
...
if(maxerror>lmaxerror)
    fprintf('Den_Optimized H_2 from %d to %d\n',maxerror,lmaxerror);
    maxerror=lmaxerror;
    bestbcr=lbcr;
    bestw=w;
else
    fprintf("Den Everything got worse\n")
end
...
if(maxerror>lmaxerror)
    fprintf('NUM_Optimized H_2 from %d to %d\n',maxerror,lmaxerror);
    maxerror=lmaxerror;
    bestbcr=lbcr;
    bestw=w;
end
