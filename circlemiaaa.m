function [bestbcr,bestw,bcr,z,wj,fz,err] = circlemiaaa(f,Z,tol,normalize,iter,ref)
...
for n=1:mmax+1
    ...
    %Compute the poles
    poles=przd(z.',w);
    %Find poles in the unit circle and replace them
    in_poles=poles(sqrt(real(poles).^2+imag(poles).^2)<1);
    while (~isempty(in_poles))

         %fprintf("%d Non-passive pole found at iteration %d\n",length(np_poles),n);
        
         %p_poles=-1*ref*real(np_poles)+1i*imag(np_poles);
         for ii=1:length(in_poles)
             al=1-sqrt(real(in_poles(ii))^2+imag(in_poles(ii))^2);
             out_pole=real(in_poles(ii))/(1-al)*(1+al)+1i*imag(in_poles(ii))/(1-al)*(1+al);
             w = w.*(z.'-out_pole)./(z.'-in_poles(ii));
         end
         poles=przd(z.',w);
         in_poles=poles(sqrt(real(poles).^2+imag(poles).^2)<1);
